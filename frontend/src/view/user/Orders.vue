          ...
          <div class="table">
            <el-table :data="ordersData" strip>
              ...
              <el-table-column prop="goodsName" label="商品名称" :show-overflow-tooltip="true">
                <template v-slot="scope">
                  <a :href="'/user/detail?id=' + scope.row.goodsId">{{scope.row.goodsName}}</a>
                </template>
              </el-table-column>
              <el-table-column prop="shopName" label="店铺名称">
                <template v-slot="scope">
                  <a :href="'/user/shop?id=' + scope.row.shopId">{{scope.row.shopName}}</a>
                </template>
              </el-table-column>
...
export default {
  ...
  methods: {
    ...
    del(id) {
      this.$confirm('是否删除','订单删除',{type: "warning"}).then(res=>{
